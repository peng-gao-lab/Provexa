...
public class Neo4jUtil {
    private static final Path databaseDirectory;

    static {
        Properties properties = new Properties();
        try (FileInputStream fis = new FileInputStream("cfg/db.properties")) {
            properties.load(fis);
            String homeDir = properties.getProperty("homeDir");
            databaseDirectory = Paths.get(homeDir, "data");
        } catch (IOException e) {
            throw new RuntimeException("Failed to load database directory from cfg/db.properties", e);
        }
    }
