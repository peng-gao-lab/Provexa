...
const CodeBlock: React.FC<CodeBlockProps> = (props) => {
  ...
  const execute = React.useCallback(async () => {
    try {
      ...
      if (props.autoDisplayGraphs) {
        const variableMatch = finalInput.match(/(\w+)\s*=\s*/);
        const variableName = variableMatch ? variableMatch[1] : null;
        if (variableName && !finalInput.includes(`display ${variableName}`)) {
          finalInput += `\ndisplay ${variableName};`;
        }
      }

      if (/db\(\)/.test(finalInput)) {
        if (!props.databaseName) {
          setOutput("Error: Please specify a database name.\nYou can do this by either entering it in the database name textbox at the top or directly in the query as db(database_name).");
          return;
        }
        finalInput = finalInput.replace(/db\(\)/, `db(${props.databaseName})`);
      }
